import { int, double, mysqlTable, varchar, json, timestamp, boolean, unique } from 'drizzle-orm/mysql-core';
...
export const userCurrencyRateTable = mysqlTable('user_currency_rate', {
  id: int().autoincrement().primaryKey(),
  userId: varchar('user_id', { length: 255 }).notNull().references(() => userTable.id, { onDelete: 'cascade' }),
  currencyId: int('currency_id').notNull().references(() => currencyTable.id, { onDelete: 'cascade' }),
  exchangeRate: double('exchange_rate').notNull(),
  lastUpdated: varchar({ length: 200 }).notNull().default(''),
}, (t) => [
  unique('user_currency_unique').on(t.userId, t.currencyId),
]);
