import { getAssetsWithTransactionsForUser } from "~/db/actions";
...
export async function loader({ request }: { request: Request }) {
  try {
    ...
    console.log("Session in assetsConfiguration loader:", session);
    if (!session || !session.user || !session.user.id) {
      return { assets: [], error: "User not authenticated" };
    }
    
    const userId = session.user.id;
    const assets = await getAssetsWithTransactionsForUser(userId);
